  ...
  switch (message) {
    ...
    case 0x00050079: {
      assert_true(!buffer_length ||
                  buffer_length == sizeof(XLIVEBASE_ASYNC_MESSAGE));
      // 454107DB
      XELOGD("XLiveBaseUnk50079({:08X}, {:08X}) Stubbed", buffer_ptr,
             buffer_length);
      return GenericMarshalled(buffer_ptr);
    }
    case 0x0005008B: {
...
X_HRESULT XLiveBaseApp::XPresenceInitialize(uint32_t buffer_ptr,
  ...
  const XLIVEBASETASK* async_task = new XLIVEBASETASK(buffer_ptr);
...
X_HRESULT XLiveBaseApp::GenresEnumerate(uint32_t buffer_ptr) {
  ...
  const uint32_t total_genre_info_size =
      genre_enumerate_request_ptr->max_count * sizeof(GENRE_INFO);
  ...
  char16_t* genre_names_ptr = reinterpret_cast<char16_t*>(
      genre_enumerate_results_ptr + genre_enumerate_request_ptr->max_count);

  for (auto const& genre : genres) {
    const uint16_t size = static_cast<uint32_t>(genre.size() + 1);

    xe::string_util::copy_and_swap_truncating(genre_names_ptr, genre.c_str(),
                                              size);
  ...
  const int32_t geners_count = static_cast<uint32_t>(genres.size());

  genre_enumerate_results_ptr->geners_total = geners_count;
  genre_enumerate_results_ptr->geners_returned = geners_count;
...
X_HRESULT XLiveBaseApp::SubscriptionEnumerate(uint32_t buffer_ptr) {
  ...
  const uint32_t subscription_info_size =
      subscription_enumerate_ptr->max_results * sizeof(SUBSCRIPTION_INFO);
  ...
  for (uint32_t i = 1; const auto& membership : memberships) {
    const uint16_t size = static_cast<uint32_t>(membership.size() + 1);
  ...
  const int32_t offers_count = static_cast<uint32_t>(memberships.size());

  subscription_enumerate_results_ptr->offers_total = offers_count;
  subscription_enumerate_results_ptr->offers_returned = offers_count;
