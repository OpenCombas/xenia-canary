...
X_HRESULT XUserFindUsersUnmarshaller::Deserialize() {
  ...
  if (!GetXLiveAsyncTask()->marshalled_request_ptr) {
    return X_E_INVALIDARG;
  }

  if (!GetXLiveAsyncTask()->results_ptr) {
    return X_E_INVALIDARG;
  }

  if (!GetXLiveAsyncTask()->results_size) {
    return X_E_INVALIDARG;
  }

  value_const_ = Read<uint32_t>();
  unkn_value_ = Read<uint64_t>();
  security_gateway_ = Read<SGADDR>();
  ...
  if (GetPosition() != xlive_async_task_ptr_->marshalled_request_size) {
