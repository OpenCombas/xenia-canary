...
DEFINE_bool(gdb_log_packets, false,
            "Log every GDB-stub packet (client requests + stub responses) at "
            "info level, for diagnosing remote-debugger issues (e.g. an empty "
            "backtrace). Off by default; works in Release builds too.",
            "General");
...
#endif
        if (cvars::gdb_log_packets) {
          XELOGI("[gdb] <- {}({})", command.cmd, command.data);
        }
